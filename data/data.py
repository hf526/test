...
def get_case():
    ...
    for j in range(1,number):  #[0.1.2]
            ...
            harder.append(eval(public.get_content(table, j, 5).encode("utf-8")))
            if harder[j-1]==t1:
                # print type(eval(public.get_content(table, j, 4).encode("utf-8")))
                data.append(eval(public.get_content(table, j, 4).encode("utf-8")))
            else:
                data.append(public.get_content(table, j, 4).encode("utf-8"))
